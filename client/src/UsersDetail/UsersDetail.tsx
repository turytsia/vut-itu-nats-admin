...
const UsersDetail = () => {
    ...
    const { operator: operatorName, account: accountName, user: userName } = useParams()
    ...
    const fetch = useCallback(
        async () => {
            ...
            try {
                const user = (await request.get.user(operatorName as string, accountName as string, userName as string) as UserPatchType & UserType)
                setUser(user)
    ...
    const onEditSubmit = async (settings: UserPatchType) => {
		try {
			...
			const response = await request.patch.user(operatorName as string, accountName as string, userName as string, settings as UserPatchType)
                        ...
                        attributes: [
                            {
                                ...
                                value: user?.nats.pub.allow ? (
                                    <span className={classes.tags}>{user?.nats.pub.allow.map(tag => <Tag key={uuid()} isBlue>{tag}</Tag>)}</span>
                                ) : null, 
                            },
                            {
                                name: "Type",
